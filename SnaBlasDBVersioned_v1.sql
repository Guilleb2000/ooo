...
BEGIN
    ...
    -- Inserción en saved_changes para cw_pole
...
BEGIN
    ...
    query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
                                top_schema, 'fo_cable', ST_AsText(real_geom), edited_by_value);	
...
-- Metodo dinamico para inserción de objetos
CREATE OR REPLACE FUNCTION insert_object(json_info jsonb) RETURNS VOID AS
$$
DECLARE
    scheme_name TEXT;
    table_name TEXT;
...
BEGIN
    ...
    LOOP
		...
		THEN
			...
			valor := 'ST_GeomFromText(' || convert_coordinates(geom_coords, geom_srid) || ', 3857)';
        ELSE
    ...
    -- Construir y ejecutar la consulta de inserción dinámica
    EXECUTE 'INSERT INTO ' || quote_ident(scheme_name) || '.' || quote_ident(table_name) || 
            ' (' || columnas || ') VALUES (' || valores || ')';
END;
$$
LANGUAGE plpgsql;

-- Metodo para cambio de coordenadas con un srid dado
CREATE OR REPLACE FUNCTION convert_coordinates(geom VARCHAR, srid INTEGER) RETURNS VARCHAR AS
$$
DECLARE
	new_geom GEOMETRY;
BEGIN
	new_geom := ST_GeomFromText(geom, srid);	
	IF srid <> 3857 THEN new_geom := ST_Transform(new_geom, 3857); END IF;
	RETURN quote_literal(ST_AsText(new_geom));
END;
$$
LANGUAGE plpgsql;
