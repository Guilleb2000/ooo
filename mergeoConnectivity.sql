...
-- JOIN security.user_roles AS r ON u.id = r.user_id
-- JOIN security.roles AS rv ON rv.id = r.role_id
JOIN security.user_authorities AS a ON u.id = a.user_id
JOIN security.authorities AS av ON av.id = a.authority_id
WHERE u.email = 'john@domain.test'
ORDER BY u.id;
...
BEGIN
   -- Creates a copy of the specified table in the new schema.
...
DECLARE
    ...
    json_data JSONB;
    action_text TEXT := 'Insert into cw_sewer_box and cw_connectivity_box';
    edited_by_value UUID;
    query_merge_value TEXT;
    insert_connectivity_box_query TEXT;
    insert_query TEXT;
    cbox_uuid UUID := gen_random_uuid();
BEGIN
    width := 3.75;
    width_cbox := 2;
    ...
    query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
                                dest_schema, 'cw_sewer_box', NEW.geom, edited_by_value);
    ...
    insert_connectivity_box_query := format(
        'INSERT INTO %I.cw_connectivity_box(id, id_gis, geom, layout_geom, diagonal_geom, edited_by) VALUES (%L, %L, %L, %L, %L, %L)',
        dest_schema,
        cbox_uuid,
        ...
        -- Inserción en saved_changes para cw_connectivity_box
    EXECUTE format('INSERT INTO %I.saved_changes(id, change_time, record_time, user_id, query_merge) VALUES ($1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $2, %L)',
                   dest_schema, insert_connectivity_box_query)
    USING cbox_uuid, edited_by_value;
...
DECLARE
    ...
    existing_change RECORD; -- Variable para verificar si existe un registro en saved_changes
BEGIN
    IF NOT ST_Equals(OLD.geom, NEW.geom) THEN
        ...
        -- Obtiene el valor de edited_by del nuevo registro de cw_sewer_box
        edited_by_value := NEW.edited_by;

        -- Verifica si edited_by_value fue obtenido correctamente
        IF edited_by_value IS NULL THEN
            RAISE EXCEPTION 'edited_by_value es NULL. Verifica el valor de edited_by en el nuevo registro de cw_sewer_box.';
        END IF;

        -- Verificar si ya existe un registro en saved_changes
        RAISE NOTICE 'OLD.id: %, OLD.id_auto: %', OLD.id, OLD.id_auto; -- RAISE NOTICE para verificar los valores
        EXECUTE format('SELECT * FROM %I.saved_changes WHERE id = $1 AND id_gis = $2', schema_name)
        INTO existing_change
        USING OLD.id, CONCAT('cw_sewer_box_', OLD.id_auto::TEXT);

        IF FOUND THEN
            -- Actualizar el registro existente en saved_changes
            EXECUTE format('UPDATE %I.saved_changes SET change_time = CURRENT_TIMESTAMP, user_id = $3, query_merge = $4 WHERE id = $1 AND id_gis = $2', schema_name)
            USING OLD.id, CONCAT('cw_sewer_box_', OLD.id_auto::TEXT), edited_by_value, action_text;
        END IF;
    END IF;       
    RETURN NEW;
END;
...
DECLARE 
    ...
    width_pole_3D FLOAT;
    dest_schema TEXT := TG_TABLE_SCHEMA;
    action_text TEXT := 'Insert into cw_pole';
    edited_by_value UUID;
    query_merge_value TEXT;
    insert_connectivity_box_query TEXT;
    insert_query TEXT;
...
BEGIN
    ...
    USING NEW.id_auto;
        ...
        -- Construcción de query_merge_value para insert_object
    query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
                                dest_schema, 'cw_pole', NEW.geom, edited_by_value);
...
DECLARE 
    ...
    current_new_geom GEOMETRY;
    current_id_gis VARCHAR;
    action_text TEXT := 'Update cw_pole'; -- Texto de acción para guardar en saved_changes
...
BEGIN
    IF NOT ST_Equals(OLD.geom, NEW.geom) THEN
        ...
        -- Obtén el valor de edited_by del nuevo registro
        edited_by_value := NEW.edited_by;
		
        IF edited_by_value IS NULL THEN
            RAISE EXCEPTION 'edited_by_value es NULL. Verifica el valor de edited_by en el nuevo registro de cw_pole.';
        END IF;
        ...
        EXECUTE format('SELECT * FROM %I.saved_changes WHERE id = $1 AND id_gis = $2', schema_name)
        INTO existing_change
        USING OLD.id, CONCAT('cw_pole_', OLD.id_auto::TEXT);

        IF FOUND THEN
            -- Actualizar el registro existente en saved_changes
            EXECUTE format('UPDATE %I.saved_changes SET change_time = CURRENT_TIMESTAMP, user_id = $3, query_merge = $4 WHERE id = $1 AND id_gis = $2', schema_name)
            USING OLD.id, CONCAT('cw_pole_', OLD.id_auto::TEXT), edited_by_value, action_text;
        END IF;
    END IF;       
    RETURN NEW;
END;
...
BEGIN
    ...
    USING CONCAT('cw_connectivity_box_', NEW.id_auto::TEXT), NEW.id_auto;
            ...
            -- Actualiza la tabla cw_connectivity_box
    EXECUTE format('UPDATE %I.saved_changes SET id_gis = $1 WHERE id = $2', dest_schema)
    USING CONCAT('cw_connectivity_box_', NEW.id_auto::TEXT), NEW.id;
...
DECLARE 
    ...
    width FLOAT;
    dest_schema TEXT := TG_TABLE_SCHEMA;
    action_text TEXT := 'Insert into cw_ground_route';
    edited_by_value UUID; -- Variable para guardar el valor de edited_by
    update_sewer_box_query TEXT;
    update_connectivity_box_query TEXT;
    update_ground_route_query TEXT;
BEGIN
    width := 0.875;
    ...
    -- Construcción de la query de ground route
    update_ground_route_query := format('UPDATE %I.cw_ground_route
        SET id_gis=CONCAT(''cw_ground_route_'', %L::text),
            layout_geom = %L
        WHERE id_auto = %L', dest_schema, NEW.id_auto::text, aux_gr_geom::text, NEW.id_auto::text);

    EXECUTE update_ground_route_query;

    -- Obtiene el valor de edited_by del nuevo registro de cw_ground_route
    ...
    -- Actualiza la tabla saved_changes para cw_ground_route
    EXECUTE format('UPDATE %I.saved_changes
        SET change_time = CURRENT_TIMESTAMP, 
            record_time = CURRENT_TIMESTAMP, 
            user_id = %L::uuid, 
            query_merge = %L
        WHERE id_gis = %L', dest_schema, edited_by_value::text, update_ground_route_query, CONCAT('cw_ground_route_', NEW.id_auto::TEXT)::text);

    -- Actualiza la tabla saved_changes para cw_sewer_box
    EXECUTE format('UPDATE %I.saved_changes
        SET change_time = CURRENT_TIMESTAMP, 
            record_time = CURRENT_TIMESTAMP, 
            user_id = %L::uuid, 
            query_merge = %L
        WHERE id_gis = %L', dest_schema, edited_by_value::text, update_sewer_box_query, id_gis_sewer);

    -- Actualiza la tabla saved_changes para cw_connectivity_box
    EXECUTE format('UPDATE %I.saved_changes
        SET change_time = CURRENT_TIMESTAMP, 
            record_time = CURRENT_TIMESTAMP, 
            user_id = %L::uuid, 
            query_merge = %L
        WHERE id_gis = %L', dest_schema, edited_by_value::text, update_connectivity_box_query, id_cb);
...
BEGIN
    ...
    -- Verificar si todos los valores no son nulos antes de insertar en saved_changes
    IF OLD.id_auto IS NOT NULL AND edited_by_value IS NOT NULL AND action_text IS NOT NULL THEN
        ...
        USING OLD.id, CONCAT('cw_ground_route_', OLD.id_auto::TEXT), edited_by_value, action_text;
    END IF;
...
BEGIN
    ...
    -- Construye el valor de query_merge_value
    query_merge_value := format('INSERT INTO %I.cw_skyway (id_gis, geom, edited_by) VALUES (%L, %L, %L)', dest_schema, CONCAT('cw_skyway_', NEW.id_auto::TEXT), NEW.geom::TEXT, edited_by_value::TEXT);
    ...
    -- Actualiza la tabla saved_changes para cw_pole
    IF id_gis_pole IS NOT NULL THEN
        EXECUTE format('UPDATE %I.saved_changes
            SET change_time = CURRENT_TIMESTAMP, 
                record_time = CURRENT_TIMESTAMP, 
                user_id = %L::uuid, 
                query_merge = %L
            WHERE id_gis = %L', dest_schema, edited_by_value::TEXT, update_pole_query, id_gis_pole::TEXT);
    END IF;
    ...
    IF id_gis_sewer IS NOT NULL THEN
        EXECUTE format('UPDATE %I.saved_changes
            SET change_time = CURRENT_TIMESTAMP, 
                ...
                user_id = %L::uuid, 
                query_merge = %L
            WHERE id_gis = %L', dest_schema, edited_by_value::TEXT, update_sewer_box_query, id_gis_sewer::TEXT);
    END IF;

    -- Actualiza la tabla saved_changes para cw_connectivity_box
    IF id_cb IS NOT NULL THEN
        EXECUTE format('UPDATE %I.saved_changes
            SET change_time = CURRENT_TIMESTAMP, 
                record_time = CURRENT_TIMESTAMP, 
                user_id = %L::uuid, 
                query_merge = %L
            WHERE id_gis = %L', dest_schema, edited_by_value::TEXT, update_connectivity_box_query, id_cb::TEXT);
    END IF;
...
DECLARE 
    ...
    schema_name TEXT := TG_TABLE_SCHEMA;
	edited_by_value UUID;
	action_text TEXT := 'Update in cw_skyway';
BEGIN
    width = 0.875;
...
DECLARE
    ...
    splice_count INT;
    dest_schema TEXT := TG_TABLE_SCHEMA;
    action_text TEXT := 'Insert into fo_splice';
    edited_by_value UUID;
    update_query TEXT;  
    query_merge_value TEXT;
...
BEGIN    
    ...
    query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
                                dest_schema, 'fo_splice', NEW.geom, edited_by_value);
...
DECLARE
    ...
    action_text TEXT := 'Insert in cw_duct';
    schema_name TEXT;
    edited_by_value UUID;
    update_query TEXT;
    query_merge_value TEXT;
BEGIN
    ...
    query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
                                schema_name, 'cw_duct', ST_AsText(NEW.geom), edited_by_value);
...
DECLARE
    ...
    insert_query TEXT;
	fiberuuid UUID := gen_random_uuid();
    edited_by_value TEXT;
BEGIN
    ...
    FOR i IN 1..n_fibers LOOP
        ...
        insert_query := format(
            'INSERT INTO %I.fo_fiber(id, id_cable, geom, layout_geom, edited_by) VALUES (%L, %L, %L::geometry, %L::geometry, %L)', 
            schema_name,
            fiberuuid,
            CONCAT('fo_cable_', cable.id_auto::text),
            ST_AsText(cable.geom),
        ...
        RAISE NOTICE 'insert_query: %', insert_query;
                ...
                -- Inserción en saved_changes para cw_connectivity_box
        EXECUTE format('INSERT INTO %I.saved_changes(id, change_time, record_time, user_id, query_merge) VALUES ($1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $2, %L)',
                   schema_name, insert_query)
        USING fiberuuid, edited_by_value;
...
BEGIN
    ...
    USING NEW.id_auto, NEW.id_auto;
        ...
        -- Actualiza la tabla cw_connectivity_box
    EXECUTE format('UPDATE %I.saved_changes SET id_gis = $1 WHERE id = $2', schema_name)
    USING CONCAT('fo_fiber_', NEW.id_auto::TEXT), NEW.id;
...
DECLARE 
    ...
    new_geom GEOMETRY;
    update_query TEXT;
    action_text TEXT := 'insert in fo_Cable';
    edited_by_value UUID;
	query_merge_value TEXT;
BEGIN
    -- Obtén el valor de edited_by
    SELECT edited_by INTO edited_by_value FROM objects.fo_cable WHERE id = NEW.id;
	
	query_merge_value := format('SELECT insert_object(%L, %L, %L, %L)',
    dest_schema, 'fo_cable', ST_AsText(NEW.geom), edited_by_value);					
    ...
    -- Llama a la función que inserta las fibras con el parámetro edited_by
    PERFORM public.create_fo_fiber(edited_by_value, NEW, new_geom, 144, dest_schema);

    -- Inserta en la tabla saved_changes
    EXECUTE format('INSERT INTO %I.saved_changes(id, id_gis, change_time, record_time, user_id, query_merge) VALUES (%L, %L, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, %L::uuid, %L)', dest_schema, NEW.id, CONCAT('fo_cable_', NEW.id_auto::TEXT), edited_by_value, query_merge_value);
...
BEGIN
    ...
    IF input_fiber_point IS NOT NULL THEN
        -- Se obtiene la cara más cercana al punto de entrada
        ...
        END;
    ELSE
        -- Se obtiene la cara más cercana al punto de salida
    ...
    IF clossest_input_face IS NOT NULL THEN      
        ...
        IF clossest_input_face = face_2 THEN
            RAISE NOTICE 'CASO 1';
            -- Se obtiene el camino más corto de la fibra a la cara
            input_fiber_to_face := ST_ShortestLine(input_fiber_point ,clossest_input_face);

            -- Se actualiza la geometría para que se conecte con el puerto.
        ...
        ELSE
            ...
            -- Punto de corte de la cara más cercana con la segunda más cercana
            inputput_faces_intersection_point := ST_Intersection(second_closest_input_face, clossest_input_face);

            -- Punto de corte de la segunda cara más cercana con la cara del puerto
    ...
    -- Llamada a la función para actualizar la topología de la fibra
    --EXECUTE format('SELECT public.update_fiber_topology($1)') USING schema_name;
END;
$$
LANGUAGE plpgsql;
...
--PROCEDIMIENTO UNIFICADO DE UPDATE
...
BEGIN
    ...
    EXECUTE format('
        UPDATE %I.%I
        SET geom = $1, locked_by = $2
        WHERE id_gis = $3', schema_name, object_type)
    USING geom, id_usuario, id_gis;
END;
$$;
